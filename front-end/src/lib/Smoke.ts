...
export class Smoke {
	...
	evolveSmoke(delta: number) {
		...
		while (smokeParticlesLength--) {
			smokeParticles[smokeParticlesLength].rotation.z += delta * 0.1;
		}
	}
	...
	addParticles() {
		...
		textureLoader.load('/smoke/smoke2.png', (texture) => {
			const smokeMaterial: any = new THREE.MeshLambertMaterial({
				color: 0xffffff,
				map: texture,
				transparent: true,
			});
			...
			while (limit--) {
				...
				this.smokeParticles = smokeMeshes;
	...
	addBackground() {
		...
		textureLoader.load(
			...
			(texture) => {
				const textMaterial: any = new THREE.MeshLambertMaterial({
